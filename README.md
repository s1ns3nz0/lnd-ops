...
The regtest, testnet, and monitoring infrastructure slices are implemented and tested on Mac arm64, including a [full project VM deletion and recreation with wallet-state monitoring](docs/evidence/mac-full-clean-wallet-state-2026-09-22.md). The Mac regtest wallet, channel, bidirectional payment, SCB, and [seed-plus-SCB recovery exercise](docs/evidence/mac-regtest-recovery-2026-09-23.md) passed. Testnet funding, the Windows execution, and live LND monitoring signal exit checks remain pending.
...
After creating and unlocking a wallet, run `ops/deploy regtest --monitoring` or `ops/deploy testnet --monitoring` to enable lndmon and the payment collector. The command requires each node's read-only macaroon and a responding LND RPC before changing the chart. It builds the collector for the host CPU and loads the image into K3s without publishing it. Adding sidecars rolls the Pod; unlock the wallet again if LND asks, then run `ops/verify <profile>` and `ops/verify-monitoring --profile <profile>`. Later ordinary `ops/deploy <profile>` invocations keep monitoring enabled. This path is prepared but has not been tested with a live wallet.
