...
test('initializer fills template project identity without overwriting existing values', async () => {
  ...
  try {
    ...
    const templateConfig = JSON.parse(await readFile(path.join(root, 'harness.config.json')));
    templateConfig.project.name = 'replace-with-project-name';
    await writeFile(path.join(directory, 'harness.config.json'), `${JSON.stringify(templateConfig)}\n`);
