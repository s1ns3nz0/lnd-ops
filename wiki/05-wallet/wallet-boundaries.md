...
집 열쇠로 자전거 자물쇠를 열 수는 없다. 금고를 여는 암호와 친구에게 보여 주는 출입증도 쓰임이 다르다. 전부 “열쇠”라고만 부르면 어느 문 앞에서 무엇을 꺼내야 하는지 헷갈린다.

**실제 시스템에 연결하면:** wallet password, seed, 백업 암호, macaroon은 서로 대체할 수 없다. 각각 지갑 잠금 해제, 키 복원, 백업 해독, API 권한에 관련된다. 실제 디지털 자격 증명은 복사될 수 있으므로 열쇠 비유처럼 원본 하나만 있다고 생각해서도 안 된다.
...
[Vault와 cert-manager를 조건부 확장으로 둔 이유](/decisions/#adr-007) · [Windows 암호화 보류와 같은 PC 백업](/decisions/#adr-010). 대화에서 정한 방향과 현재 구현·보류 범위를 함께 읽는다.
