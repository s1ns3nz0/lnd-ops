...
SCB의 static은 매 결제의 잔액 변화를 복사한다는 뜻이 아니다. 복구에 필요한 채널 정보를 보관하며 채널 집합이 바뀌면 백업도 갱신되어야 한다. 따라서 “백업이 한 시간 전이다”라는 시간 정보와 “현재 필요한 채널 정보가 반영됐다”는 최신성은 별개다. 이 프로젝트가 백업 존재·최신성·나이를 나누어 보는 이유다. [LND SCB 복구](https://docs.lightning.engineering/lightning-network-tools/lnd/disaster-recovery)
