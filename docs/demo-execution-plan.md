...
- **Complete:** Windows Phase 7 constrained kagent. During a real regtest peer-isolation fault, the external Ollama-backed agent observed the active signal and produced a runbook-grounded diagnosis; the exact policy and active channel recovered, one named probe restart succeeded with audit evidence, an immediate repeat hit cooldown, and a wallet-unlock request was denied. `ops/phase7-acceptance` passed on runtime revision `f331e41add0381c8b9e520e1d91ccc59617ac73e`.
