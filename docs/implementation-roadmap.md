...
**Primary acceptance test:** use two complementary proofs on each prepared host. First, start a wallet-free disposable environment with no K3s cluster, project namespaces, Helm releases, PVCs, or locally tagged project images; the Mac may also start without a Lima VM. Clone the repository and run `ops/doctor`, `ops/bootstrap`, `ops/deploy regtest`, and `ops/deploy testnet` as documented. These commands must create every required Kubernetes resource without hand-created resources or edits to generated manifests. Second, preserve operator-created wallets and their PVCs while completing the explicit seed recording, read-only macaroon, testnet funding, peer/channel, payment, SCB, and recovery gates. Prove ordinary redeployment by reapplying the charts and comparing the preserved identity and state; wallet recreation is not required for this proof. Run both proofs separately on Mac and Windows.
...
**Roadmap acceptance criterion:** Can a prepared Mac or Windows host with no project resources run the repository scripts and reach a working node and monitoring stack? A stage passes only when its scripts recreate its resources from that empty state and its functional checks pass. Record the exact cleanup and rerun commands, host, result, and any manual wallet or funding gate. A deployment that works only because resources already exist does not pass.

**Decision rule for every implementation stage:** Delete all disposable resources created by that stage, run its scripts again, and check the resulting behavior. If the cleanup, fresh run, or functional check is missing or fails, mark the stage **pending** even when an incremental deployment succeeds. Keep the cleanup scope and any protected funded data explicit in the evidence.

Use the [clean-start runbook](clean-start-runbook.md) for host-specific cleanup, rerun commands, manual gates, and evidence fields.
...
**Stage completion baseline:** a successful run against an existing cluster is insufficient for infrastructure reproducibility. Remove and recreate wallet-free disposable Kubernetes resources and locally tagged project images on both hosts, and record the cleanup, rerun, and secret-free result. Once a wallet exists, preserve its PVC and reuse that wallet for functional and ordinary redeployment checks. Do not require wallet recreation or delete a wallet-bearing PVC to prove a stage rerun. Compare node identity, channels, SCB, PVC, and monitoring history across chart reapplication instead. For stage 0, repeat the preflight and image-platform checks from a clean local image tag state. For stage 1, also remove the disposable K3s cluster; on Mac, verify creation when the Lima VM is absent.

The proof sequence is: (1) use a dedicated project cluster with no unrelated workloads; remove the `lnd-regtest`, `lnd-testnet`, and `lnd-monitoring` namespaces, their Helm releases/PVCs, the project cluster where required, and locally tagged `localhost/lnd-ops-payment-collector` and `python:lnd-ops-verified-*` images; (2) recreate and verify the wallet-free stack with the documented scripts; (3) create wallets once in that recreated stack; and (4) preserve those wallets for all functional checks. Docker build layers, upstream image cache, Lima downloads, and registry content are allowed caches. Ordinary redeployment means Helm chart reapplication and the resulting Pod recreation; it never means Helm uninstall, namespace deletion, K3s recreation, or PVC deletion. Node identity and PVC UID must remain identical. Channel state and SCB may change only through an explicit channel or recovery operation, and the host SCB must match the current LND source when compared. Prometheus continuity is proved by retrieving the same timestamp, value, and series identity after redeployment.

Run the seed+SCB recovery exercise once per required host. It may copy the encrypted SCB from that host into an isolated recovery PVC and requires the operator to enter that same host's seed interactively. Never copy a live wallet database or PVC contents between Mac and Windows. Each host uses a different wallet identity; only an original node and a recovery node derived from the same seed are forbidden from running concurrently.
...
| 5. Clean-start and redeploy proof | Add CI checks for chart render, policy syntax, scripts, and both image platforms. Add a clean-host runbook and `ops/redeploy-check` that records node key, channels, PVC UID, SCB checksum, and monitoring history before and after `ops/deploy testnet`. | On each prepared host, scripts build the wallet-free stack from zero Kubernetes resources. The operator then creates the host's wallets once and completes the documented seed, macaroon, funding, peer, channel, and payment gates. `ops/redeploy-check` reuses that funded wallet, reruns both charts, and shows the node identity, channels, PVC, SCB copy, and monitoring history remain intact; secret-free evidence is saved. **MVP complete.** |
...
Each stage adds its own script checks and Linux container-based validation. A stage is complete only after its clean-state rerun and functional exit check pass on both target machines; a successful Helm command alone is insufficient.
