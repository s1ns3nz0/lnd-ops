...
| 5. Rebuild proof | Add CI checks for chart render, policy syntax, scripts, and both image platforms. Add a clean-host runbook and `ops/redeploy-check` that records identity, channels, PVCs, SCB, and monitoring history before and after `ops/deploy`. | A fresh checkout on each prepared host follows the documented commands; repeated deployment leaves both funded nodes and monitoring data intact; the evidence checklist is saved without secrets. **MVP complete.** |
