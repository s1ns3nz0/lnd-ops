...
The Mac arm64 testnet proof remains a Phase 8 cross-platform requirement. Windows Secure Boot, Device Encryption, and the isolated seed-plus-SCB recovery exercise remain Phase 5 work. The test-only payer PVC is retained for future incoming-payment evidence, but its StatefulSet is disabled after the final repeat deployment because the canonical persistent testnet profile runs one primary node.
